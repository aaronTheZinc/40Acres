...
import { RegisterInputSchema, type TRegisterInput } from "@/schema/auth";
...
export const POST = postRouteValidator<TRegisterInput>(
  RegisterInputSchema,
  async (req) => {
        ...
        if (error instanceof PrismaClientKnownRequestError) {
          ...
          if (error.code === "P2002") {
            return NextResponse.json(
              { err: `${error.meta?.target ?? "field"} exists already` },
              failed,
            );
          }
